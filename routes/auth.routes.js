...
router.post("/user/upload", uploader.single("imageUrl"), (req, res, next) => {
  let pictureUrl = "";
  req.file
...
router.patch("/user/edit", isLoggedIn, (req, res, next) => {
  let email = req.session.loggedInUser.email;
  const { country, favirote } = req.body;

  User.findByIdAndUpdate({ email: email }, { country, favirote }, { new: true })
    .then((response) => {
      req.session.loggedInUser= response
